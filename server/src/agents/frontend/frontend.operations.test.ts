...
import { previewFrontendOperations } from './frontend.operations';

describe('previewFrontendOperations', () => {
  ...
  it("repairs an 'update' on a path that doesn't exist into a 'create'", async () => {
    ...
    const { operationsWithDiff } = await previewFrontendOperations(owner, 'p1', [
      { type: 'update', path: 'src/App.tsx', content: 'export default function App() {}', reason: 'wire it up' },
    ]);

    expect(operationsWithDiff[0].type).toBe('create');
    expect(operationsWithDiff[0].content).toBe('export default function App() {}');
  ...
  it("leaves an 'update' on a path that does exist unchanged", async () => {
    ...
    const { operationsWithDiff } = await previewFrontendOperations(owner, 'p1', [
      { type: 'update', path: 'src/App.tsx', content: 'new content', reason: 'edit it' },
    ]);
