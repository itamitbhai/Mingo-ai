...
import { BatchOperationInput, BatchOperationType, IFrontendOperation, IOperationPreviewResult, LockType } from 'shared';
...
/**
 * Bridges the Testing Agent's validated operations to Phase 4's Virtual Filesystem — mirrors
 * `agents/database/database.operations.ts` exactly. The Testing Agent never spawns a process or
 * touches a real OS filesystem here; every write here targets the *workspace* (generated test source
 * files), through the same `preview.service`/`batch.service`/`snapshot.service`/`lock.service`
 * machinery every other agent uses. Actually *running* those tests is a separate concern —
 * `testing.runner.ts`, which reads the applied files back out through the sandbox's own materializer,
...
export interface TestingOperationsPreview {
  preview: IOperationPreviewResult;
  operationsWithDiff: IFrontendOperation[];
}

/** The model occasionally assumes a test file already exists when it doesn't and proposes an
 *  "update" for it — repaired into a "create" here rather than failing the whole task, mirroring
 *  `database.operations.ts`'s `normalizeCreateVsUpdate`. */
 ...
 * client can render a diff with zero extra requests. Mirrors `database.operations.ts`'s
 * `previewDatabaseOperations`.
 */
export async function previewTestingOperations(
  owner: Types.ObjectId,
  projectId: string,
  rawOperations: IFrontendOperation[]
): Promise<TestingOperationsPreview> {
...
/**
 * Applies a validated batch of Testing Agent operations as one logical workspace change: acquires an
 * `agent` lock on every existing target file, snapshots the workspace, applies the batch via
 * `batch.service.applyBatch`, and restores the snapshot if the apply itself throws partway through.
 * Mirrors `database.operations.ts`'s `applyDatabaseOperations`.
 */
export async function applyTestingOperations(
  owner: Types.ObjectId,
  projectId: string,
  taskId: string,
  operations: IFrontendOperation[]
  ...
  try {
    const snapshot = await snapshotService.createSnapshot(
      owner,
      projectId,
      `Before Testing Agent task ${taskId}`,
    ...
    try {
      ...
      logger.error('testing_agent.apply.failed', { projectId, taskId, error: err instanceof Error ? err.message : err });
      await snapshotService.restoreSnapshot(owner, projectId, snapshot.id).catch((restoreErr) => {
        logger.error('testing_agent.rollback.failed', { projectId, taskId, error: restoreErr });
      });
      throw err;
    }
  } finally {
