...
- Four agents exist today: the Frontend Agent (tasks with "type": "frontend" or "recommendedAgent": "frontend"), the Backend Agent ("backend"/"backend"), the Database Agent ("database"/"database"), and the Testing Agent ("testing"/"testing"). Each is real and buildable — schedule them normally, with a testing task depending on every frontend/backend/database task whose output it actually covers, so it never runs before the code it tests exists.
- Tasks of any OTHER type ("authentication" when not explicitly assigned "recommendedAgent": "backend", "devops", "security" as a task type, "setup", "integration", "configuration", "documentation", "deployment") still have NO agent to build them — they will sit forever, and any task that "depends on" one of them will be permanently blocked, since its dependency can never complete. Prefer folding authentication/configuration work into a "backend"-typed task with "recommendedAgent": "backend" when that's genuinely where it belongs, rather than leaving it as an unbuildable type.
