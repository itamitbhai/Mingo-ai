...
import { previewDatabaseOperations } from './database.operations';

describe('previewDatabaseOperations', () => {
  ...
  it("repairs an 'update' on a path that doesn't exist into a 'create'", async () => {
    ...
    const { operationsWithDiff } = await previewDatabaseOperations(owner, 'p1', [
      { type: 'update', path: 'server/database/connection.js', content: "module.exports = require('mongoose');", reason: 'wire it up' },
    ]);

    expect(operationsWithDiff[0].type).toBe('create');
    expect(operationsWithDiff[0].content).toBe("module.exports = require('mongoose');");
  ...
  it("leaves an 'update' on a path that does exist unchanged", async () => {
    ...
    const { operationsWithDiff } = await previewDatabaseOperations(owner, 'p1', [
      { type: 'update', path: 'server/models/Todo.js', content: 'new content', reason: 'edit it' },
    ]);
  ...
  it('leaves a create operation on a path that truly does not exist unchanged', async () => {
    ...
    const { operationsWithDiff } = await previewDatabaseOperations(owner, 'p1', [
      { type: 'create', path: 'server/models/Todo.js', content: 'x', reason: 'new file' },
    ]);
  ...
  it("repairs a 'create' on a path that already exists into an 'update'", async () => {
    ...
    const { operationsWithDiff } = await previewDatabaseOperations(owner, 'p1', [
      { type: 'create', path: 'server/models/Todo.js', content: 'new content', reason: 'todo model' },
    ]);
